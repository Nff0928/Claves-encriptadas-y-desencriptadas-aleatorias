...
def codificar_clave():
    ...
    clave_generada = label_clave.cget("text")
    resultado_codificado = codificar_cesar(clave_generada, corrimiento)

    resultado_area.delete('1.0', tk.END)
    resultado_area.insert(tk.END, resultado_codificado)

def decodificar_clave():
    corrimiento = int(corrimiento_field.get())
...
label_clave.grid(row=2, column=0, columnspan=2, padx=10, pady=5)

corrimiento_label = tk.Label(root, text="Corrimiento:")
corrimiento_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")

corrimiento_field = tk.Entry(root)
corrimiento_field.grid(row=3, column=1, padx=10, pady=5)

boton_codificar = tk.Button(root, text="Codificar", command=codificar_clave)
boton_codificar.grid(row=4, column=0, padx=10, pady=5)

boton_descodificar = tk.Button(root, text="Descodificar", command=decodificar_clave)
boton_descodificar.grid(row=4, column=1, padx=10, pady=5)

resultado_label = tk.Label(root, text="Resultado:")
resultado_label.grid(row=5, column=0, padx=10, pady=5, sticky="w")

resultado_area = tk.Text(root, height=5, width=40)
resultado_area.grid(row=6, column=0, columnspan=2, padx=10, pady=5)
